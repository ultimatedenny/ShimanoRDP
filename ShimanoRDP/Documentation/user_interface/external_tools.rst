...
External Tools can help you get things done that can't be done in ShimanoRDP.
...
But from within ShimanoRDP you can launch applications and tell them what to do with the use of arguments, parameters and variables
...
Arguments
   ...
   And also which variables from ShimanoRDP to use for the arguments.
...
This is the list of variables supported by ShimanoRDP:
...
ShimanoRDP will also expand environment variables such as %PATH% and %USERPROFILE%. If you need to use an environment
variable with the same name as an ShimanoRDP variable, use \\% instead of %. The most common use of this is for the
...
If you need to send a variable name to a program without ShimanoRDP expanding it, use ^% instead of %.
ShimanoRDP will remove the caret (^) and leave the rest unchanged.
