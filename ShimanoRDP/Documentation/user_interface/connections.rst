...
The connections dialog is the main collection of all connections that inside ShimanoRDP.
...
.. note::
    ...
    Don't forget that ShimanoRDP will save the change on exit auto unless you have unchecked this setting in options.
...
There are multiple ways to open a connection in ShimanoRDP,
...
- Close the panel with the (Which leaves your session active on server but closes connection in ShimanoRDP)
- Close the connection tab with (Also keeps your login active on server but closes RDP connection in ShimanoRDP)
- Double click the connection tab (Same as above where the connection is active on server but closes RDP connection in ShimanoRDP)
