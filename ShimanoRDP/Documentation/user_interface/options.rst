...
where you can personalize your options for all of ShimanoRDP.
This includes how to set logging, credentials and so on.
Continue reading for the details of the different options here.

Startup/Exit
============
Options below are for the various settings for Startup/Exit of ShimanoRDP.
...
.. list-table::
   ...
   * - Save connection on exit
     - On
     - Save to connection file/database on exit of ShimanoRDP
   ...
   * - Allow only a single instance of the application (ShimanoRDP restart required)
     - Off
     - Enforces and makes sure only a single instance of ShimanoRDP is running on the computer

Appearance
==========
Various options for ShimanoRDP appearance.
...
.. list-table::
   ...
   * - Language
     ...
     - Which language to use for the interface of ShimanoRDP
   ...
   * - Show full connections file path in window title
     - Off
     - Adds the complete path to the title of ShimanoRDP to where the connection file is located
   * - Always show notification area icon
     - Off
     - Adds ShimanoRDP to the taskbar in the OS
   * - Minimize to notification area
     - Off
     - Will place ShimanoRDP in taskbar on minimize

Tabs & Panels
=============
Various settings for how tabs & panels should work in ShimanoRDP.
...
.. list-table::
   ...
   * - Always show panel tabs
     - Off
     - Will always show the tabs & panels in ShimanoRDP
   * - Open new tab to the right of the currently selected tab
     - On
     - When active then open next tab on the right of the active selection in ShimanoRDP. Turn this off and next tab will open the next connection at the end of all tabs
   ...
   * - Create a empty panel when ShimanoRDP starts
     - Off
     - On startup if this is active ShimanoRDP will create a panel mentioned under Panel Name
...
.. list-table::
   ...
   * - Set hostname like display name when creating or renaming connections
     - Off
     - Will make ShimanoRDP try to use the remote host hostname to set the title of the tab in ShimanoRDP.
   * - Save connections after every exit
     - On
     - When active ShimanoRDP will save the connection tree to the active config after every exit. If inactive then you have to save using **File > Save Connection File** or keyboard shortcut Ctrl+S
   ...
   * - When closing connections
     ...
     - Various options of how ShimanoRDP should act when you close connections. The different options are listed below:
       ::

           - Warn me when closing connections
           - Warn me only when closing multiple connections
           - Warn me only when exiting ShimanoRDP
...
Credentials
===========
Options for credentials in ShimanoRDP. The main purpose here is that when you have empty username, password or domain field then use below information.
...
Updates
=======
Options for how ShimanoRDP should check for updates from the website.
...
.. list-table::
   ...
   * - Check for updates at startup
     - On (Every 14 days)
     - Here you can choose how often ShimanoRDP checks for updates. Standard is every 14 days
   * - Release Channel
     - Stable
     - The main channel to use for ShimanoRDP. Note that the channels are described under the selection. Stable is suggested for normal usage but its always good to get feedback on upcoming releases
...
Theme
=====
This is not enabled by default but can be used inside ShimanoRDP.
To enable themes you have to first enable it in the checkbox at the bottom of the options.
Then restart ShimanoRDP in order for it to work.
