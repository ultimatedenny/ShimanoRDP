...
==================================
- Create a new Database called "ShimanoRDP" on your SQL Server.
- Run the SQL Script for your DB type listed below in topic (SQL Table creation Scripts) on the newly created Database.
- Give the users that you want to grant access to the ShimanoRDP Connections Database Read/Write permissions on the Database.

Steps to configure ShimanoRDP for SQL
====================================
- Start ShimanoRDP if it's not already running.
...
- Click OK to apply the changes. The main window title should now change to "ShimanoRDP | SQL Server".
...
- You should now be able to do everything you were able to do with the XML storage plus see the changes live on another ShimanoRDP instance that is connected to the same Database.
