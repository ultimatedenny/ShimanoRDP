...
namespace ShimanoRDP.UI.Tabs
{
    /// <summary>
    /// This class is lifted from VS2013DockPaneStrip from DockPanelSuite and customized for ShimanoRDP
    /// </summary>
    [ToolboxItem(false)]
    internal class DockPaneStripNG : DockPaneStripBase
    {
        private class ShimanoRDPTab : Tab
        {
            public ShimanoRDPTab(IDockContent content)
                : base(content)
            {
            }
        ...
        {
            return new ShimanoRDPTab(content);
        }
        ...
        private static GraphicsPath GraphicsPath => ShimanoRDPAutoHideStrip.GraphicsPath;
        ...
        private bool CalculateDocumentTab(Rectangle rectTabStrip, ref int x, int index)
        {
            ...
            if (!(Tabs[index] is ShimanoRDPTab tab)) return false;
        ...
        private void CalculateTabs_Document()
        {
            ...
            {
                var tempX = x;
                if (Tabs[m_startDisplayingTab] is ShimanoRDPTab tab)
        ...
        private bool EnsureDocumentTabVisible(IDockContent content, bool repaint)
        {
            ...
            if (Tabs[index] is ShimanoRDPTab tab && tab.TabWidth != 0)
                return false;
        ...
        private Rectangle GetTabRectangle_ToolWindow(int index)
        {
            ...
            var tab = (ShimanoRDPTab)Tabs[index];
        ...
        private void DrawTab(Graphics g, ShimanoRDPTab tab)
        {
            if (Appearance == DockPane.AppearanceStyle.ToolWindow)
        ...
        private void DrawTab_ToolWindow(Graphics g, ShimanoRDPTab tab)
        {
        ...
        private void DrawTab_Document(Graphics g, ShimanoRDPTab tab)
        {
            if (tab.Rectangle == null) return;
            var rect = tab.Rectangle.Value;
            if (tab.TabWidth == 0)
        ...
        protected override void OnMouseMove(MouseEventArgs e)
        {
            ...
            if (index != -1)
            {
                if (Tabs[index] is ShimanoRDPTab tab)
                {
                    if (Appearance == DockPane.AppearanceStyle.ToolWindow ||
                        Appearance == DockPane.AppearanceStyle.Document)
                    {
